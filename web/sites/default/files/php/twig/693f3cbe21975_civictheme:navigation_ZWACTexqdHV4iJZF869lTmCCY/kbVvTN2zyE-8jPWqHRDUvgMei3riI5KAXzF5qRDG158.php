<?php

use Twig\Environment;
use Twig\Error\LoaderError;
use Twig\Error\RuntimeError;
use Twig\Extension\CoreExtension;
use Twig\Extension\SandboxExtension;
use Twig\Markup;
use Twig\Sandbox\SecurityError;
use Twig\Sandbox\SecurityNotAllowedTagError;
use Twig\Sandbox\SecurityNotAllowedFilterError;
use Twig\Sandbox\SecurityNotAllowedFunctionError;
use Twig\Source;
use Twig\Template;
use Twig\TemplateWrapper;

/* civictheme:navigation */
class __TwigTemplate_e50d2a6fd406c4c8d1fb50702df0a139 extends Template
{
    private Source $source;
    /**
     * @var array<string, Template>
     */
    private array $macros = [];

    public function __construct(Environment $env)
    {
        parent::__construct($env);

        $this->source = $this->getSourceContext();

        $this->parent = false;

        $this->blocks = [
            'title_block' => [$this, 'block_title_block'],
        ];
        $this->sandbox = $this->extensions[SandboxExtension::class];
        $this->checkSecurity();
    }

    protected function doDisplay(array $context, array $blocks = []): iterable
    {
        $macros = $this->macros;
        // line 1
        yield $this->extensions['Drupal\Core\Template\TwigExtension']->renderVar($this->extensions['Drupal\Core\Template\TwigExtension']->attachLibrary("core/components.civictheme--navigation"));
        yield $this->extensions['Drupal\Core\Template\TwigExtension']->renderVar($this->extensions['Drupal\Core\Template\ComponentsTwigExtension']->addAdditionalContext($context, "civictheme:navigation"));
        yield $this->extensions['Drupal\Core\Template\TwigExtension']->renderVar($this->extensions['Drupal\Core\Template\ComponentsTwigExtension']->validateProps($context, "civictheme:navigation"));
        // line 34
        yield "
";
        // line 35
        $context["menu_id"] = ((array_key_exists("menu_id", $context)) ? (Twig\Extension\CoreExtension::default(($context["menu_id"] ?? null), "navigation")) : ("navigation"));
        // line 36
        $context["dropdown_class"] = Twig\Extension\CoreExtension::sprintf("ct-navigation--%s", ((array_key_exists("type", $context)) ? (Twig\Extension\CoreExtension::default(($context["type"] ?? null), "none")) : ("none")));
        // line 37
        $context["variant_class"] = (((($tmp = ($context["variant"] ?? null)) && $tmp instanceof Markup ? (string) $tmp : $tmp)) ? (Twig\Extension\CoreExtension::sprintf("ct-navigation--%s", ($context["variant"] ?? null))) : (""));
        // line 38
        $context["theme_class"] = Twig\Extension\CoreExtension::sprintf("ct-theme-%s", ((array_key_exists("theme", $context)) ? (Twig\Extension\CoreExtension::default(($context["theme"] ?? null), "light")) : ("light")));
        // line 39
        $context["modifier_class"] = Twig\Extension\CoreExtension::sprintf("%s %s %s %s", ($context["theme_class"] ?? null), ($context["dropdown_class"] ?? null), ($context["variant_class"] ?? null), ((array_key_exists("modifier_class", $context)) ? (Twig\Extension\CoreExtension::default(($context["modifier_class"] ?? null), "")) : ("")));
        // line 40
        yield "
";
        // line 41
        if ((($tmp =  !Twig\Extension\CoreExtension::testEmpty(($context["items"] ?? null))) && $tmp instanceof Markup ? (string) $tmp : $tmp)) {
            // line 42
            yield "  ";
            $context["menu_level_classes"] = [];
            // line 43
            yield "  ";
            if (CoreExtension::inFilter(($context["type"] ?? null), ["dropdown", "drawer"])) {
                // line 44
                yield "    ";
                $context['_parent'] = $context;
                $context['_seq'] = CoreExtension::ensureTraversable(Twig\Extension\CoreExtension::keys(($context["items"] ?? null)));
                foreach ($context['_seq'] as $context["_key"] => $context["key"]) {
                    // line 45
                    yield "      ";
                    if ((($tmp = CoreExtension::getAttribute($this->env, $this->source, (($_v0 = ($context["items"] ?? null)) && is_array($_v0) || $_v0 instanceof ArrayAccess && in_array($_v0::class, CoreExtension::ARRAY_LIKE_CLASSES, true) ? ($_v0[$context["key"]] ?? null) : CoreExtension::getAttribute($this->env, $this->source, ($context["items"] ?? null), $context["key"], [], "array", false, false, true, 45)), "below", [], "any", false, false, true, 45)) && $tmp instanceof Markup ? (string) $tmp : $tmp)) {
                        // line 46
                        yield "        ";
                        // line 47
                        yield "        ";
                        $context["item_attributes"] = $this->extensions['Drupal\Core\Template\TwigExtension']->createAttribute(["data-collapsible" => "", "data-collapsible-collapsed" => "", "data-collapsible-icon-group" => "", "data-collapsible-group" =>                         // line 51
($context["menu_id"] ?? null), "data-collapsible-duration" => (((($tmp =                         // line 52
($context["is_animated"] ?? null)) && $tmp instanceof Markup ? (string) $tmp : $tmp)) ? ("250") : ("0"))]);
                        // line 54
                        yield "
        ";
                        // line 55
                        if ((($context["type"] ?? null) == "drawer")) {
                            // line 56
                            yield "          ";
                            // line 57
                            yield "          ";
                            $context["item_dropdown_columns_class"] = (((($tmp = ($context["dropdown_columns"] ?? null)) && $tmp instanceof Markup ? (string) $tmp : $tmp)) ? (Twig\Extension\CoreExtension::sprintf("ct-navigation__dropdown-columns--%s", ($context["dropdown_columns"] ?? null))) : (""));
                            // line 58
                            yield "          ";
                            $context["item_dropdown_columns_fill_class"] = (((($tmp = ($context["dropdown_columns_fill"] ?? null)) && $tmp instanceof Markup ? (string) $tmp : $tmp)) ? ("ct-navigation__dropdown-columns--fill") : (""));
                            // line 59
                            yield "          ";
                            $context["item_modifier_class"] = Twig\Extension\CoreExtension::sprintf("ct-navigation__has-dropdown %s %s", ($context["item_dropdown_columns_class"] ?? null), ($context["item_dropdown_columns_fill_class"] ?? null));
                            // line 60
                            yield "          ";
                            $context["menu_level_classes"] = [1 => "container"];
                            // line 61
                            yield "          ";
                            // line 62
                            yield "          ";
                            $context["new_item"] = Twig\Extension\CoreExtension::merge((($_v1 = ($context["items"] ?? null)) && is_array($_v1) || $_v1 instanceof ArrayAccess && in_array($_v1::class, CoreExtension::ARRAY_LIKE_CLASSES, true) ? ($_v1[$context["key"]] ?? null) : CoreExtension::getAttribute($this->env, $this->source, ($context["items"] ?? null), $context["key"], [], "array", false, false, true, 62)), ["attributes" => ($context["item_attributes"] ?? null), "modifier_class" => ($context["item_modifier_class"] ?? null)]);
                            // line 63
                            yield "          ";
                            $context["items"] = Twig\Extension\CoreExtension::merge(($context["items"] ?? null), [$context["key"] => ($context["new_item"] ?? null)]);
                            // line 64
                            yield "        ";
                        } else {
                            // line 65
                            yield "          ";
                            // line 66
                            yield "          ";
                            $context["item_modifier_class"] = "ct-navigation__has-dropdown";
                            // line 67
                            yield "          ";
                            // line 68
                            yield "          ";
                            $context["new_item"] = Twig\Extension\CoreExtension::merge((($_v2 = ($context["items"] ?? null)) && is_array($_v2) || $_v2 instanceof ArrayAccess && in_array($_v2::class, CoreExtension::ARRAY_LIKE_CLASSES, true) ? ($_v2[$context["key"]] ?? null) : CoreExtension::getAttribute($this->env, $this->source, ($context["items"] ?? null), $context["key"], [], "array", false, false, true, 68)), ["attributes" => ($context["item_attributes"] ?? null), "modifier_class" => ($context["item_modifier_class"] ?? null)]);
                            // line 69
                            yield "          ";
                            $context["items"] = Twig\Extension\CoreExtension::merge(($context["items"] ?? null), [$context["key"] => ($context["new_item"] ?? null)]);
                            // line 70
                            yield "        ";
                        }
                        // line 71
                        yield "      ";
                    }
                    // line 72
                    yield "    ";
                }
                $_parent = $context['_parent'];
                unset($context['_seq'], $context['_key'], $context['key'], $context['_parent']);
                $context = array_intersect_key($context, $_parent) + $_parent;
                // line 73
                yield "  ";
            } else {
                // line 74
                yield "    ";
                $context['_parent'] = $context;
                $context['_seq'] = CoreExtension::ensureTraversable(Twig\Extension\CoreExtension::keys(($context["items"] ?? null)));
                foreach ($context['_seq'] as $context["_key"] => $context["key"]) {
                    // line 75
                    yield "      ";
                    if ((($tmp = CoreExtension::getAttribute($this->env, $this->source, (($_v3 = ($context["items"] ?? null)) && is_array($_v3) || $_v3 instanceof ArrayAccess && in_array($_v3::class, CoreExtension::ARRAY_LIKE_CLASSES, true) ? ($_v3[$context["key"]] ?? null) : CoreExtension::getAttribute($this->env, $this->source, ($context["items"] ?? null), $context["key"], [], "array", false, false, true, 75)), "below", [], "any", false, false, true, 75)) && $tmp instanceof Markup ? (string) $tmp : $tmp)) {
                        // line 76
                        yield "        ";
                        $context["items_without_below"] = Twig\Extension\CoreExtension::merge((($_v4 = ($context["items"] ?? null)) && is_array($_v4) || $_v4 instanceof ArrayAccess && in_array($_v4::class, CoreExtension::ARRAY_LIKE_CLASSES, true) ? ($_v4[$context["key"]] ?? null) : CoreExtension::getAttribute($this->env, $this->source, ($context["items"] ?? null), $context["key"], [], "array", false, false, true, 76)), ["below" => null]);
                        // line 77
                        yield "        ";
                        $context["items"] = Twig\Extension\CoreExtension::merge(($context["items"] ?? null), [$context["key"] => ($context["items_without_below"] ?? null)]);
                        // line 78
                        yield "      ";
                    }
                    // line 79
                    yield "    ";
                }
                $_parent = $context['_parent'];
                unset($context['_seq'], $context['_key'], $context['key'], $context['_parent']);
                $context = array_intersect_key($context, $_parent) + $_parent;
                // line 80
                yield "  ";
            }
            // line 81
            yield "
  <div class=\"ct-navigation ";
            // line 82
            yield $this->extensions['Drupal\Core\Template\TwigExtension']->escapeFilter($this->env, ($context["modifier_class"] ?? null), "html", null, true);
            yield "\" ";
            if ((array_key_exists("attributes", $context) &&  !(null === ($context["attributes"] ?? null)))) {
                yield $this->extensions['Drupal\Core\Template\TwigExtension']->escapeFilter($this->env, ($context["attributes"] ?? null), "html", null, true);
            }
            yield ">
    ";
            // line 83
            if ((($tmp = ($context["title"] ?? null)) && $tmp instanceof Markup ? (string) $tmp : $tmp)) {
                // line 84
                yield "      ";
                yield from $this->unwrap()->yieldBlock('title_block', $context, $blocks);
                // line 92
                yield "    ";
            }
            // line 93
            yield "
    <nav
      class=\"ct-navigation__items\"
      ";
            // line 96
            if ((($tmp =  !Twig\Extension\CoreExtension::testEmpty(($context["name"] ?? null))) && $tmp instanceof Markup ? (string) $tmp : $tmp)) {
                yield "aria-label=\"";
                yield $this->extensions['Drupal\Core\Template\TwigExtension']->escapeFilter($this->env, ($context["name"] ?? null), "html", null, true);
                yield "\"";
            }
            // line 97
            yield "    >
    ";
            // line 98
            yield from $this->load("civictheme:menu", 98)->unwrap()->yield(CoreExtension::toArray(["theme" =>             // line 99
($context["theme"] ?? null), "items" =>             // line 100
($context["items"] ?? null), "modifier_class" => "ct-navigation__menu", "menu_level_modifier_class" =>             // line 102
($context["menu_level_classes"] ?? null)]));
            // line 104
            yield "    </nav>
  </div>
";
        }
        $this->env->getExtension('\Drupal\Core\Template\TwigExtension')
            ->checkDeprecations($context, ["type", "variant", "theme", "items", "is_animated", "dropdown_columns", "dropdown_columns_fill", "attributes", "title", "name"]);        yield from [];
    }

    // line 84
    /**
     * @return iterable<null|scalar|\Stringable>
     */
    public function block_title_block(array $context, array $blocks = []): iterable
    {
        $macros = $this->macros;
        // line 85
        yield "        ";
        yield from $this->load("civictheme:heading", 85)->unwrap()->yield(CoreExtension::toArray(["theme" =>         // line 86
($context["theme"] ?? null), "level" => 2, "content" =>         // line 88
($context["title"] ?? null), "modifier_class" => "ct-navigation__title"]));
        // line 91
        yield "      ";
        yield from [];
    }

    /**
     * @codeCoverageIgnore
     */
    public function getTemplateName(): string
    {
        return "civictheme:navigation";
    }

    /**
     * @codeCoverageIgnore
     */
    public function isTraitable(): bool
    {
        return false;
    }

    /**
     * @codeCoverageIgnore
     */
    public function getDebugInfo(): array
    {
        return array (  231 => 91,  229 => 88,  228 => 86,  226 => 85,  219 => 84,  210 => 104,  208 => 102,  207 => 100,  206 => 99,  205 => 98,  202 => 97,  196 => 96,  191 => 93,  188 => 92,  185 => 84,  183 => 83,  175 => 82,  172 => 81,  169 => 80,  163 => 79,  160 => 78,  157 => 77,  154 => 76,  151 => 75,  146 => 74,  143 => 73,  137 => 72,  134 => 71,  131 => 70,  128 => 69,  125 => 68,  123 => 67,  120 => 66,  118 => 65,  115 => 64,  112 => 63,  109 => 62,  107 => 61,  104 => 60,  101 => 59,  98 => 58,  95 => 57,  93 => 56,  91 => 55,  88 => 54,  86 => 52,  85 => 51,  83 => 47,  81 => 46,  78 => 45,  73 => 44,  70 => 43,  67 => 42,  65 => 41,  62 => 40,  60 => 39,  58 => 38,  56 => 37,  54 => 36,  52 => 35,  49 => 34,  45 => 1,);
    }

    public function getSourceContext(): Source
    {
        return new Source("", "civictheme:navigation", "themes/contrib/civictheme/components/03-organisms/navigation/navigation.twig");
    }
    
    public function checkSecurity()
    {
        static $tags = ["set" => 35, "if" => 41, "for" => 44, "block" => 84, "include" => 98];
        static $filters = ["default" => 35, "format" => 36, "keys" => 44, "merge" => 62, "escape" => 82];
        static $functions = ["create_attribute" => 47];

        try {
            $this->sandbox->checkSecurity(
                ['set', 'if', 'for', 'block', 'include'],
                ['default', 'format', 'keys', 'merge', 'escape'],
                ['create_attribute'],
                $this->source
            );
        } catch (SecurityError $e) {
            $e->setSourceContext($this->source);

            if ($e instanceof SecurityNotAllowedTagError && isset($tags[$e->getTagName()])) {
                $e->setTemplateLine($tags[$e->getTagName()]);
            } elseif ($e instanceof SecurityNotAllowedFilterError && isset($filters[$e->getFilterName()])) {
                $e->setTemplateLine($filters[$e->getFilterName()]);
            } elseif ($e instanceof SecurityNotAllowedFunctionError && isset($functions[$e->getFunctionName()])) {
                $e->setTemplateLine($functions[$e->getFunctionName()]);
            }

            throw $e;
        }

    }
}
